...
module.exports = {
  ...
  compute: (req, res) => {
    let params = req.body;
    let file = 'src/json/payrollPayRulesPercentage.json';

    jsonfile.readFile(file, (err, data) => {
      let basic = params.basic;
      let regularDay = computationTools(basic, params.regular_day, data.workingDay.regularDay);
      let specialDay = computationTools(basic, params.special_day, data.workingDay.specialDay);
      let specialDayFord = computationTools(basic, params.special_day_ford, data.workingDay.specialDayFord);
      let holiday = computationTools(basic, params.holiday, data.workingDay.holiday);
      let holidayFord = computationTools(basic, params.holiday_ford, data.workingDay.holidayFord);
      let doubleHoliday = computationTools(basic, params.double_holiday, data.workingDay.doubleHoliday);
      let doubleHolidayFord = computationTools(basic, params.double_holiday_ford, data.workingDay.doubleHolidayFord);
...
const computationTools = (basic, numberOfDays, percentage) => {
  let finalComputation = 0.00;
  let dailyRate = (basic / 26) * percentage;
